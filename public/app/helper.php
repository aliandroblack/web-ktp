...
function getNilai($idData, $kolom, $idUnit = 0) {
  if($idUnit == 0 || !isset($idUnit) || trim($idUnit) === "") {
    $nilai = DB::table("data_detil_nilai")
      ->where("fid_data_detil", $idData)
      ->where("kolom", $kolom)
      ->get();  
  }
  else {
    $nilai = DB::table("data_detil_nilai")
      ->where("fid_data_detil", $idData)
      ->where("kolom", $kolom)
      ->where("fid_unit", $idUnit)
      ->get();    
  }

  if(count($nilai) <= 0)  return "";

  if($nilai[0]->{"nilai_tipe"} == "DECIMAL") {
    return $nilai[0]->{"nilai_decimal"};
  }
  elseif($nilai[0]->{"nilai_tipe"} == "VARCHAR") {
    return $nilai[0]->{"nilai_varchar"};
  }
  else {
    return "";
  }
...
function getSetting($setId)  {
  $setValue = "";
  $setting = DB::select("SELECT set_value FROM trv_settings WHERE set_id = ?",array($setId));
  if(count($setting) > 0) {
...
function validateReferenceEntry($refCategory, $entryValue) {
  /*
  $cek = DB::select("SELECT * FROM dlh_referensi WHERE R_KATEGORI = ? AND R_ID = ?", array($refCategory, $entryValue));
